...
export class MentorController {
  async createOrUpdateProfile(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const {
        company, currentRole, experience, expertise,
        linkedIn, github, hourlyRate, languages,
      } = req.body;

      const mentor = await Mentor.findOneAndUpdate(
        { user: userId },
        { $set: { company, currentRole, experience, expertise, linkedIn, github, hourlyRate, languages } },
